...
@paddle.no_grad()
def do_speed(config):
    current_device = paddle.device("gpu")
    ...
    print("Loading model:", model.set_state_dict(state_dict))
...
def do_flop(config):
    ...
    total_flops = paddle.to_tensor(total_flops)
    ...
    logger.info(
        "Total GFlops: {:.1f}±{:.1f}".format(
            paddle.mean(total_flops).item() / 1e9, paddle.std(total_flops).item() / 1e9
        )
    )
