...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(data_root_dir, "imdb")):
        os.makedirs(os.path.join(data_root_dir, "imdb"))
    output_file = os.path.join(
        data_root_dir, "imdb", "{}.npy".format(imdb_name)
    )
    ...
    if data_split != "test":
        ...
        with open(
            os.path.join(
                data_root_dir, "vocab/{}_vocab.txt".format(imdb_name)
            ),
            "w",
        ) as f:
