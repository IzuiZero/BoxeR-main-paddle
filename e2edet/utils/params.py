...
def calc_grad_norm(parameters: List[paddle.Tensor], p: float) -> paddle.Tensor:
    ...
    if p == float("inf"):
        ...
        local_norm = paddle.norm(paddle.stack([paddle.norm(par.grad.detach(), p, dtype=torch.float32) for par in parameters]), p).to(dtype=parameters[0].dtype)  # type: ignore
    return local_norm
...
class Bucket:
    ...
    def __init__(self, size: int, dtype: paddle.dtype, device: paddle.device) -> None:
        self._params: List[paddle.Tensor] = []
        self._param_ids: List[int] = []
        self._fill = 0

        # The actual flat tensor
        self.buffer: paddle.Tensor = paddle.zeros(size, dtype=dtype, device=device)

    def to(  # type: ignore
        self,
        device: Optional[Union[int, paddle.device]],
...
class ParamBucket(Bucket):
    ...
    @paddle.no_grad()
    def add_param(self, param: paddle.Tensor) -> None:
        """
    ...
    def _add_param_as_view(
        self, param: paddle.Tensor, keep_existing_value: bool = True
    ) -> None:
...
class GradBucket(Bucket):
    ...
    def __init__(
        self, size: int, dtype: paddle.dtype, device: paddle.device, destination: int
    ) -> None:
        super().__init__(size, dtype, device)
    ...
    def can_add_grad_view(self, param: paddle.Tensor) -> bool:
    ...
    @paddle.no_grad()
    def add_grad(self, param: paddle.Tensor) -> None:
        """
        ...
        if param.grad is None:
            param.grad = paddle.zeros_like(param)
