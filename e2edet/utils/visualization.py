...
def draw_bbox3d(image, bbox_vertices, color=(0, 200, 200), thickness=1):
    for idx in range(bbox_vertices.shape[0] - 1):
        v1 = (int(bbox_vertices[idx][0].item()), int(bbox_vertices[idx][1].item()))
        v2 = (int(bbox_vertices[idx + 1][0].item()), int(bbox_vertices[idx + 1][1].item()))
...
def draw_line(image, v1, v2, color=(0, 200, 200), thickness=1):
...
def draw_bbox2d(image, bbox2d, color=(0, 200, 200), thickness=1):
...
def imshow(image, attn=None, figure_num=None):
...
def imsave(file_name, image, figure_num=None):
    if figure_num is not None:
        plt.figure(figure_num)
    else:
        f, axs = plt.subplots(1, 1, figsize=(15, 15))
