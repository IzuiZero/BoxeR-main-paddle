...
def load_pretraind_state_dict(state_dict, trainer, strict=False):
    ...
    print(trainer.model.set_state_dict(model_dict, strict=strict))

    if "optimizer" in state_dict:
        trainer.optimizer.set_state_dict(state_dict["optimizer"])
    else:
        warnings.warn(
    ...
    if "lr_scheduler" in state_dict:
        trainer.lr_scheduler.set_state_dict(state_dict["lr_scheduler"])
    else:
        warnings.warn(
    ...
    if "grad_scaler" in state_dict and trainer.grad_scaler is not None:
        trainer.grad_scaler.set_state_dict(state_dict["grad_scaler"])
...
class Checkpoint:
    def __init__(self, trainer):
        ...
        self.pth_filepath = os.path.join(
            self.ckpt_foldername, self.ckpt_prefix + self.model_name + "_final.pdparams"
        )
        ...
        if tp.resume:
            if tp.resume_file is not None:
                ...
                ckpt_file_paths = sorted(
                    glob.glob(os.path.join(self.models_foldername, "model_*.pdparams")),
    ...
    def _paddle_load(self, file):
        return paddle.load(file)
    ...
    def save(self, update):
        ...
        ckpt_filepath = os.path.join(self.models_foldername, "model_%d.pdparams" % update)
        ...
        paddle.save(ckpt, ckpt_filepath)
    ...
    def finalize(self):
        if is_master():
            paddle.save(self.trainer.model.state_dict(), self.pth_filepath)
