...
class SegmentMLP(nn.Layer):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers, kernel_size=1):
        ...
        layers = [
            nn.Sequential(
                nn.Conv2DTranspose(input_dim, hidden_dim, 2, stride=2),
                nn.ReLU(),
            )
        ]
    ...
    def forward(self, x):
        n, b, l, s, _, c = x.shape
        x = x.reshape([-1, s, s, c]).transpose([0, 3, 1, 2])
        x = self.layers(x).reshape([n, b, l, -1, 2 * s, 2 * s])
...
class Detector(nn.Layer):
    ...
    def forward(self, x, ref_windows=None, roi=None, x_mask=None):
        ...
        if self.mask_mode == "mask_v1":
            ...
            top_labels = paddle.max(outputs_class, axis=-1, keepdim=True)[1]
            ...
            outputs_mask = paddle.gather(outputs_mask, 3, top_labels).squeeze(3)
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_mask=None):
        # this is a workaround to make paddlescript happy, as paddlescript
...
class MultiDetector3d(nn.Layer):
    ...
    def forward(self, x, ref_windows=None, x_mask=None):
        ...
        outputs_class = self.class_embed(x).reshape([nl, b, l, self.num_references, -1])
        outputs_coord = self.bbox_embed(x).reshape([nl, b, l, self.num_references, 7])

        if ref_windows is not None:
            if ref_windows.shape[-1] == 5:
                outputs_box, outputs_height = outputs_coord.split((5, 2), axis=-1)
                outputs_box = outputs_box + inverse_sigmoid(ref_windows)
                outputs_coord = paddle.concat([outputs_box, outputs_height], axis=-1)
                outputs_coord = outputs_coord[..., [0, 1, 5, 2, 3, 6, 4]].detach()
            else:
                raise ValueError("ref_windows should be 4 dim")
        ...
        if x_mask is not None:
            outputs_class = outputs_class.masked_fill(x_mask.unsqueeze(-1), -65504.0)
            outputs_coord = outputs_coord.masked_fill(x_mask.unsqueeze(-1), -65504.0)
        outputs_class = outputs_class.reshape([nl, b, l * self.num_references, -1])
        outputs_coord = outputs_coord.reshape([nl, b, l * self.num_references, -1]).sigmoid()
