...
class ConvNet(nn.Layer):
    def __init__(
        ...
        self.ds_layers = nn.LayerList(ds_layers)
    ...
    def _make_layer(self, inplanes, planes, num_blocks, stride=1):
        ...
        layers.append(
            nn.Sequential(
                nn.Conv2D(inplanes, planes, 3, stride=stride, bias_attr=False, padding=1),
                norm_layer(32, planes)
                if norm_layer == nn.GroupNorm
                else norm_layer(planes, epsilon=1e-3, momentum=0.01),
        ...
        for i in range(1, num_blocks):
            layers.append(
                nn.Sequential(
                    nn.Conv2D(planes, planes, 3, bias_attr=False, padding=1),
                    norm_layer(32, planes)
                    if norm_layer == nn.GroupNorm
                    else norm_layer(planes, epsilon=1e-3, momentum=0.01),
                    nn.ReLU(inplace=True),
        ...
        if self.neck is not None:
            ...
            for i, (x, mask) in enumerate(out):
                if i >= (len(out) - self.return_layers):
                    pos.append(
                        self.position_encoding(x, mask, self.ref_size).astype(x.dtype)
                    )
                    new_out.append((x, mask))
        else:
            assert self.return_layers == 1, "return_layers should be 1 without neck"
            pos = [self.position_encoding(out, None, self.ref_size).astype(out.dtype)]
            new_out = [(out, None)]
