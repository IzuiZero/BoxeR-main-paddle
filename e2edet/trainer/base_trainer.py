...
import paddle
from paddle.cuda.amp import GradScaler
...
class BaseTrainer:
    ...
    def _set_device(self):
        ...
        if is_distributed() and self.config.distributed.init_method is not None:
            self.device = paddle.device.set_device("gpu:{}".format(self.local_rank))
    ...
    def load_model_and_optimizer(self):
        ...
        if "cuda" in str(self.device):
            device_info = "CUDA Device {} is: {}".format(
                ...
                paddle.device.get_device_name(self.local_rank),
            )
    ...
    def _parallelize_model(self):
        self.parallel = False
        if is_distributed() and "cuda" in str(self.device):
    ...
    def _init_params_and_checkpoint(self):
        self.writer.write("Paddle version is: " + paddle.__version__)
