...
def evaluate(split, trainer):
    ...
    for _ in range(len(dataloader)):
        ...
        if accumulated_results is not None:
            accumulated_results.update(
                recursive_copy_to_device(results, True, paddle.device("cpu"))
            )
    ...
    if accumulated_results is not None:
        ...
        if is_master():
            ...
            if iou_type is None:
                ...
                paddle.save(merged_results, save_file)
    ...
    if "cuda" in str(trainer.device):
        paddle.fluid.dygraph.dygraph_utils.empty_cache()
    ...
    for idx in range(len(trainer.dataloaders["train"])):
        ...
        if iter_per_update > 1:
            ...
            num_boxes = paddle.to_tensor(
                [num_boxes], dtype="float32", device=trainer.device
            )
            if is_distributed():
                paddle.distributed.all_reduce(num_boxes)
            num_boxes = paddle.clip(num_boxes / get_world_size(), min=1)
...
def _forward(split, batch, model, trainer):
    ...
    if trainer.use_fp16 and split == "train":
        ...
        dtype = "bfloat16" if trainer.use_fp16 == "bfloat16" else "float16"
        with paddle.amp.auto_cast(enable=True, dtype=dtype):
...
def _step(optimizer, max_norm, current_update, trainer):
    ...
    if paddle.isnan(norm).item() or paddle.isinf(norm).item():
...
def _update_info(split, output, trainer, finished_batch=True):
    ...
    if (
        ...
        if "cuda" in str(trainer.device):
            stats["max mem"] = paddle.fluid.dygraph.memory_stats(
                trainer.device
            ).max_alloc / 1000
            stats["max mem"] //= 1000
