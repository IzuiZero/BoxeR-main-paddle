...
import paddle
from paddle.distributed import get_rank, get_world_size, get_group
...
if TYPE_CHECKING:  
    from paddle.optimizer.optimizer import _params_t
else:
    _params_t = Any
...
def _gpu_capabilities_older_than_50() -> bool:
    ...
    if _gpu_is_old is None:
        for i in range(paddle.device.get_device_count()):
            major, minor = paddle.fluid.core.get_cuda_compute_capability(i)
            if major <= 5:
                _gpu_is_old = True
        if _gpu_is_old is None:
            _gpu_is_old = False
...
def _broadcast_object(
    ...
    dist_device: paddle.CUDAPlace = paddle.CUDAPlace(0),
) -> Any:
    """
    ...
    """
    if paddle.distributed.get_rank() == src_rank:
        # Emit data
        buffer = io.BytesIO()
        paddle.save(obj, buffer)
        data = bytearray(buffer.getbuffer())
        length_tensor = paddle.to_tensor([len(data)], place=dist_device)
        data_send_tensor = paddle.to_tensor(data, place=dist_device)
        broadcast(length_tensor, src=src_rank, group=group)
        broadcast(data_send_tensor, src=src_rank, group=group)
    else:
        # Fetch from the source
        length_tensor = paddle.zeros([1], dtype='int64', place=dist_device)
        broadcast(length_tensor, src=src_rank, group=group)
        data_recv_tensor = paddle.zeros([int(length_tensor.item())], dtype='uint8', place=dist_device)
        broadcast(data_recv_tensor, src=src_rank, group=group)
        buffer = io.BytesIO(data_recv_tensor.numpy())
        obj = paddle.load(buffer)
    return obj

class OSS(paddle.optimizer.Optimizer):
    """
    Wraps an arbitrary :class:`optim.Optimizer <paddle.optimizer.Optimizer>`
    ...
    def __init__(
        self,
        params: _params_t,
        optim: Type[paddle.optimizer.Optimizer] = paddle.optimizer.SGD,
        ...
        **default: Any,
        ...
        self.__per_device_params: Dict[
            paddle.device, List[List[paddle.Tensor]]
        ] = OrderedDict()  # device, rank, params
        self.__param_rank: Dict[paddle.Tensor, int] = {}
        self._partition_parameters: List[List[dict]] = []
        self.__param_to_index: Dict[int, int] = {}
        self.__local_params: Optional[List[paddle.Tensor]] = None

        # Default empty values + immutables
        self._optim_defaults = default
        self._optim_constructor = optim

        self.group = group if group is not None else get_group()
        self.world_size = get_world_size(self.group)
        self.backend = paddle.distributed.get_backend(self.group)
        ...
        self.buckets: Dict[paddle.device, Dict[int, ParamBucket]] = {}
        self._all_states: List[
            Dict[str, Any]
        ] = []  # Optional consolidated optimizer state
        self._default_device = paddle.device("cpu")
