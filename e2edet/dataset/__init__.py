...
def build_dataloader(config, dataset_type, dataset):
    ...
    if get_world_size() > 1:
        if cache_mode:
            ...
            # Ensure the correct import for PaddlePaddle's distributed sampler
            from paddle.io import DistributedBatchSampler
            other_args["sampler"] = DistributedBatchSampler(
                dataset, shuffle=other_args["shuffle"]
            )
...
for file in os.listdir(datasets_dir):
    ...
    if (
        ...
        # Ensure the correct import for PaddlePaddle's dynamic module loading
        module = __import__(f"e2edet.dataset.{dataset_name}", globals(), locals(), [])