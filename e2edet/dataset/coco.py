...
class COCODetection(BaseDataset):
    ...
    def format_for_evalai(self, output, targets, threshold=None, return_rles=False):
        ...
        target_sizes = paddle.stack([t["orig_size"] for t in targets], axis=0)
        ...
        def _process_output(indices, sizes, boxes, ins_scores, ins_masks=None):
            ...
            boxes = paddle.gather(boxes, 1, top_boxes.unsqueeze(-1).repeat(1, 1, 4))

            img_h, img_w = paddle.unbind(sizes, axis=1)
            ...
            if ins_masks is not None:
                ...
                ins_masks = paddle.gather(ins_masks, 1, top_boxes)
                ...
                for i, (cur_mask, cur_size) in enumerate(zip(ins_masks, sizes)):
                    ...
                    pred_mask = (mask >= 0.5).cast('float32')
        ...
        if threshold is None:
            topk_values, topk_indexes = paddle.topk(prob, 100, axis=1, largest=False)
            scores = topk_values
            ...
            indices = paddle.arange(
                top_pred.shape[1], dtype='int64', device=out_logits.device
            )
            results = []
            for i in range(out_logits.shape[0]):
                if not paddle.any(top_pred[i]).item():
                    result = (
                ...
                top_indexes = paddle.masked_select(indices, top_pred[i])
                scores = paddle.masked_select(prob[i], top_pred[i])
