...
import math
import numpy as np
from e2edet.utils.distributed import is_dist_avail_and_initialized, get_rank, get_world_size
from e2edet.utils.det3d.general import box_collision_test
from e2edet.utils.det3d.box_ops import center_to_corner_box2d, rotate_points_along_z
...
class BatchSampler:
    ...
    def __init__(self, sampled_list, name=None, shuffle=True):
        ...
        self.rank = rank
        self.num_samples = int(math.ceil(len(sampled_list) * 1.0 / self.
            num_replicas))
...
class DataBaseSampler:
    ...
    def __init__(self, db_infos, groups, min_points=0, difficulty=-1, rate=1.0
        ...
        if min_points > 0 or difficulty > -1:
            ...
            for name, db_info in db_infos.items():
                new_db_infos[name] = [info for info in db_info if info[
                    'num_points_in_gt'] > min_points and info['difficulty'] >=
                    difficulty]
    ...
    def sample_all(self, root_path, gt_boxes, gt_names, num_point_features):
        ...
        for class_name, max_sample_num in zip(self._sample_classes, self.
            _sample_max_nums):
            sampled_num = int(max_sample_num - np.sum([(n == class_name) for
                n in gt_names]))
        ...
        for class_name, sampled_num in zip(sampled_groups, sample_num_per_class
            ...
            if sampled_num > 0:
                sampled_cls = self.sample_class(class_name, sampled_num,
                    avoid_coll_boxes)
                sampled += sampled_cls
                if len(sampled_cls) > 0:
                    if len(sampled_cls) == 1:
                        sampled_gt_box = sampled_cls[0]['box3d_lidar'][np.
                            newaxis, ...]
                    else:
                        sampled_gt_box = np.stack([s['box3d_lidar'] for s in
                            sampled_cls], axis=0)
                    sampled_gt_boxes += [sampled_gt_box]
                    avoid_coll_boxes = np.concatenate([avoid_coll_boxes,
                        sampled_gt_box], axis=0)
        if len(sampled) > 0:
            ...
            for info in sampled:
                ...
                try:
                    s_points = np.fromfile(info_path, dtype=np.float32
                        ).reshape(-1, num_point_features)
                    if 'rot_transform' in info:
                        rot = info['rot_transform']
                        s_points[:, :3] = rotate_points_along_z(s_points[np
                            .newaxis, :, :], rot[np.newaxis, :])[0]
            ...
            ret = {'gt_names': np.array([s['name'] for s in sampled]),
                ...
                'gt_boxes': sampled_gt_boxes, 'points': np.concatenate(
                s_points_list, axis=0)}
            ret['group_ids'] = np.arange(gt_boxes.shape[0], gt_boxes.shape[
                0] + len(sampled))
        else:
            ret = None
        return ret
    ...
    def sample(self, name, num):
        ret = self._sampler_dict[name].sample(shape=num)
        return ret

    def sample_class(self, name, num, gt_boxes):
        sampled = self._sampler_dict[name].sample(shape=num)
        sampled = copy.deepcopy(sampled)
        num_gt = gt_boxes.shape[0]
        num_sampled = len(sampled)
        gt_boxes_bv = center_to_corner_box2d(gt_boxes[:, 0:2], gt_boxes[:, 
            3:5], gt_boxes[:, -1])
        ...
        sp_boxes_bv = center_to_corner_box2d(sp_boxes_new[:, 0:2],
            sp_boxes_new[:, 3:5], sp_boxes_new[:, -1])
